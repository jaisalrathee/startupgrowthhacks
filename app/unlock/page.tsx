...
export default async function UnlockPage() {
  ...
  if (!user) redirect("/signup?next=/unlock");
  if (user.isPro) redirect("/dashboard");

  const items = [
    "All 464 deep-dive growth hacks — hand-illustrated, with the how, example & gotcha",
    ...
    "Download as Notion, PDF, and Airtable for your team",
  ];

  return (
    <div style={{ maxWidth: 620, margin: "0 auto", padding: "48px 20px 80px" }}>
      <div className="paywall" style={{ marginTop: 0 }}>
        ...
        <form action={purchaseUnlockAction}>
          <button type="submit" className="btn primary" style={{ padding: "14px 28px", fontSize: 15, fontWeight: 700 }}>
            Pay £49 — instant access →
          </button>
        </form>
        <div className="mono" style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap", marginTop: 16, color: "var(--text-faint)", fontSize: 10 }}>
          <span>✓ 30-day refund</span><span>✓ Lifetime access</span><span>✓ Pay once, no renewals</span>
        </div>
        <p style={{ marginTop: 18, fontSize: 11, color: "var(--text-faint)" }}>
          Demo build — checkout is simulated. In production this routes through Stripe.
        </p>
      </div>
    </div>
  );
